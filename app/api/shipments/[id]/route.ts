...
import { shipmentsStore, ShipmentStatus, TrackingEvent } from "@/lib/shipments";
...
export async function PATCH(
  ...
  const shipment = shipmentsStore.get(trackingNumber);
...
export async function DELETE(
  ...
  if (!shipmentsStore.has(trackingNumber)) {
    return NextResponse.json({ error: "Shipment not found." }, { status: 404 });
  }

  shipmentsStore.delete(trackingNumber);
